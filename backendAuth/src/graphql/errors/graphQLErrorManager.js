...
import {
  ErrorCapaDatos,
  ErrorDatosNoEncontrados,
  ErrorSeguridad,
  ErrorValidacionDatos,
} from "../../models/errors/errorsManager.js";
import { FabricaErrores } from "../../models/errors/errorsManager.js";
import { dbConnectionProvider } from "../../models/db/db.js";
...
const errorsFormatter = (formattedError, error) => {
  ...
  if (
    ...
    return {
      ...formattedError,
      message:
        "Operacion Invalida de acuerdo al Graphql schema definido para este sistema",
    };
  }
  if (formattedError.extensions.code === ApolloServerErrorCode.BAD_USER_INPUT) {
    return {
      ...formattedError,
      message:
        "La consulta graphql enviada tiene un valor invalido en uno de los argumentos especificados",
    };
  }
  if (
    formattedError.extensions.code ===
    ...
    return {
      ...formattedError,
      message:
        "El hash enviado para un query graphql almacenado no fue encontrado en el cache APQ",
    };
  }
  if (
    formattedError.extensions.code ===
    ...
    return {
      ...formattedError,
      message:
        "El request incluye varias consultas nombradas pero no especifico cual ejecutar; o la consulta nombrada no fue incluida en el request",
    };
  }
  if (formattedError.extensions.code === ApolloServerErrorCode.BAD_REQUEST) {
    return {
      ...formattedError,
      message:
        "Ocurrio un error antes de que el servidor pudiera intentar interpretar la consulta graphql enviada",
    };
  }

  loggerGlobal.warn(
    "Hasta ahora no va ninguno de los errores tipificados de Graphql ..."
  );
  ...
  if (
    backendError instanceof ErrorSeguridad ||
    (backendError.message &&
      backendError.message.toUpperCase().includes("ERROR DE SEGURIDAD"))
  ) {
    loggerGlobal.error(
      "Capture un Error de Seguridad: " + backendError.message
    );
    let originalErrorMessage = "";
  ...
  if (
    ...
    (errorCause instanceof dbConnectionProvider.pgpErrors.QueryResultError ||
      ...
      errorCause instanceof
        dbConnectionProvider.pgpErrors.PreparedStatementError ||
      errorCause instanceof
        dbConnectionProvider.pgpErrors.ParameterizedQueryError)
  ) {
    loggerGlobal.error(
      "Capture un Error de la capa de datos lanzado por pgp No debidamente envuelto con los errores del sistema ..."
    );
